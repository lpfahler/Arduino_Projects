...
void setup() {
  ...
  // setup for LEDs on Pins 4-7 - stored in array ledPins
...
void loop() {
  ...
  // if the reading is zero (or almost zero) turn off the note
  // AND do not turn on the LED associated with the playingNote
  if (potReading < MIN_POT_READING) {
    ...
    // turn on LED associated with playingNote
    digitalWrite(ledPins[playingNote], HIGH);
  }
  ...
  delay(60000/tempo);
  // turn off LED associate with playingNote
  digitalWrite(ledPins[playingNote], LOW);
  playingNote++;
